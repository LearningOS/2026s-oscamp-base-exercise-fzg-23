...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    let fz=Command::new("sh").args(["-c", command]).status().unwrap().code().unwrap_or(-1);
    fz
}
...
pub fn run_command_with_result(program: &str, args: &[&str]) -> io::Result<String> {
   ...
   let mut output=Command::new(program).args(args).stdout(Stdio::piped()).output()?;
   String::from_utf8(output.stdout).map_err(|_|io::Error::new(io::ErrorKind::InvalidData, "invalid utf-8"))
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    let mut child=Command::new("grep").stdout(Stdio::piped()).stdin(Stdio::piped()).arg(pattern).spawn().unwrap();
    let mut stdin=child.stdin.take().unwrap();
    for line in input.lines(){
        writeln!(stdin, "{}", line);
    };
    drop(stdin);
    let mut stdout=child.wait_with_output().unwrap();
