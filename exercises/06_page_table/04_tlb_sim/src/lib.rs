...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
      for entry in &self.tlb.entries{
          if entry.asid==self.current_asid && entry.vpn==vpn {
      ...
      for i in &self.page_table  {
          if i.0==self.current_asid && i.1.vpn==vpn {
              self.tlb.insert(vpn,i.1.ppn,self.current_asid,i.1.flags);
              return Some(i.1.ppn);
          };
      };
        None
