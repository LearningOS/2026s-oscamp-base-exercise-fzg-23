...
impl Sv39PageTable {
            ...
            if level==0{
              let pte=ppn<<PPN_SHIFT|flags;
                ...
                if pte&PTE_V==0{
                    ...
                    let node = self.nodes.get_mut(&current_ppn).unwrap();
                    node.entries[index]=new_pte;
                    current_ppn=new_ppn;
                }else{
                    current_ppn=(pte >> PPN_SHIFT) & ((1u64 << 44) - 1);
                }
            }
        }
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
       ...
       for level in (0..3).rev(){
           ...
           if level==0{
               let pte =node.entries[index];
               if pte&(PTE_R|PTE_W|PTE_X)!=0 && pte&PTE_V!=0{
                   let ppn = (pte >> PPN_SHIFT) & ((1u64 << 44) - 1);
                   let page_offset = va & 0xFFF; // 取低12位
                   return TranslateResult::Ok((ppn << 12) | page_offset);
               }
               else{
               ...
               if pte&(PTE_R|PTE_W|PTE_X)!=0{
                   let ppn = (pte >> PPN_SHIFT) & ((1u64 << 44) - 1);
                   let page_offset = va & 0x1FFFFF; // 取低12位
                   return TranslateResult::Ok((ppn << 12) | page_offset);
               }
               else{
                   current_ppn=(pte >> PPN_SHIFT) & ((1u64 << 44) - 1);
